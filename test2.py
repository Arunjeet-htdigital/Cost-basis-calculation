...
# ── Buy ───────────────────────────────────────────────────────────────────────
def process_buy(d30p, asset, date, quantity_n, price_n):
    """
...
# ── Sell ──────────────────────────────────────────────────────────────────────
def process_sell(d30p, graveyard, gains, asset, date, quantity, price,
    ...
    if not pool:
        gains.append({**source_row,
                      'timestamp': date, 'asset': asset, 'kind': kind,
                      'quantity': quantity, 'price': price,
                      'proceeds': quantity * price, 'cost_basis': 0.0,
                      'avco': 0.0, 'gain': quantity * price,
    ...
    if quantity > total_quantity + 1e-9:
        gains.append({**source_row,
                      ...
                      'proceeds': quantity * price, 'cost_basis': 0.0,
                      'avco': avco, 'gain': 0.0,
                      'error': f'sell qty {quantity:.8f} > pool total {total_quantity:.8f}'})
        return
    ...
    while remaining > 1e-12 and len(pool) > 1:
        ...
        if remaining >= front_qty - 1e-12:
            remaining -= front_qty
            graveyard[asset].append(pool.popleft())
        else:
...
def process_gas(d30p, graveyard, gains, row, source_row, order_type):
    ...
    process_sell(d30p, graveyard, gains,
...
def run_matching(df: pd.DataFrame):
    d30p      = defaultdict(deque)
    graveyard = defaultdict(list)
    gains     = []
        ...
        if order_type in ('deposit', 'buy'):
            ...
            if _is_valid(asset) and _is_valid(qty) and _is_valid(price):
                try:
                    ...
                    if qty_f > 0:
                        process_buy(d30p, str(asset).strip(), ts, qty_f, price_f)
                except (ValueError, TypeError):
                    pass
        ...
        # ── TRADE ─────────────────────────────────────────────────────────
            ...
            if _is_valid(in_asset) and _is_valid(in_qty) and _is_valid(in_price):
                try:
                    ...
                    if qty_f > 0:
                        process_buy(d30p, str(in_asset).strip(), ts, qty_f, price_f)
                except (ValueError, TypeError):
                    pass
...
def summarise_pools(d30p, graveyard):
    ...
    for asset, pool in d30p.items():
        ...
        rows.append({
            ...
            'live_lots':      len(pool),
            'exhausted_lots': len(graveyard.get(asset, [])),
        })
...
if __name__ == '__main__':
    df = run(r"C:\Users\Arunjeet\OneDrive - Harris & Trotter LLP\Downloads\test_keeper")

    print("\nRunning AVCO matching...")
    gains, d30p, graveyard = run_matching(df)
    ...
    graveyard.to_csv(r"sold_lots.csv", index=False)
